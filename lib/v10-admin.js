...
function registerV10Admin({ app, auth, uuid }) {
  ...
  app.post(
    ...
    asyncRoute(async (req, res) => {
      ...
      const days = Math.max(
        1,
        Number(req.body.days || selectedPackage?.duration_days || 30)
      );
      ...
      const payload = {
        ...
        max_mt5_accounts: Math.max(
          1,
          Number(
            req.body.maxMt5Accounts ||
            selectedPackage?.max_mt5_accounts ||
            1
          )
        ),
  ...
  console.log(
    `DEWA SMC V10.1 Supabase Admin: ${supabase ? "ON" : "OFF"}`
  );
}
