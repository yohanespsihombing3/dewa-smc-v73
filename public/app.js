...
const $=id=>document.getElementById(id),PRESETS={crypto:["BTCUSDT","ETHUSDT","SOLUSDT","XRPUSDT","ADAUSDT"],forex:["EUR/USD","GBP/USD","USD/JPY","AUD/USD","USD/CAD"],gold:["XAU/USD","XAG/USD"],hybrid:["BTCUSDT","ETHUSDT","XAU/USD","EUR/USD"]};
...
function showChangePassword(){$("changePasswordCard").classList.remove("hidden")}async function changePassword(){try{await api("/api/auth/change-password",{method:"POST",body:JSON.stringify({password:$("newPassword").value})});$("changePasswordCard").classList.add("hidden");log("Password berhasil diganti.");await loadMe()}catch(e){log(e.message)}}function saveLocks(){localStorage.setItem("DEWA_V6_LOCKS",JSON.stringify(locks))}function saveLastDir(){localStorage.setItem("DEWA_LAST_DIR",JSON.stringify(DEWA_LAST_DIR))}function symbols(){return $("symbols").value.split(",").map(x=>x.trim().toUpperCase()).filter(Boolean)}function sleep(ms){return new Promise(r=>setTimeout(r,ms))}function applyMarket(){let m=$("market").value;if(m!=="custom")$("symbols").value=PRESETS[m].join(",")}function stop(){if(autoTimer)clearInterval(autoTimer);autoTimer=null;queueRunning=false;$("scanStatus").textContent="Stopped";log("Stopped")}function resetSignals(){locks={};saveLocks();render()}function start(){stop();scanQueue();autoTimer=setInterval(scanQueue,Number($("refresh").value)*1000);$("scanStatus").textContent="Auto ON"}function isCryptoSymbol(s){return !s.includes("/")&&!s.includes(":")}function tfLabel(){let v=$("tf").value;return v==="60"?"1H":v==="240"?"4H":v+"m"}function getHTFTf(){let tf=String($("tf").value);if(tf==="5")return"60";if(["15","30","60"].includes(tf))return"240";if(tf==="240")return"D";return"60"}function intervalFromTf(src,tf){if(src==="binance"){return tf==="5"?"5m":tf==="15"?"15m":tf==="30"?"30m":tf==="60"?"1h":tf==="240"?"4h":tf==="D"?"1d":"5m"}return tf==="5"?"5min":tf==="15"?"15min":tf==="30"?"30min":tf==="60"?"1h":tf==="240"?"4h":tf==="D"?"1day":"5min"}async function fetchCandles(symbol,customTf=null){let src=isCryptoSymbol(symbol)?"binance":"twelvedata",path=src==="binance"?"/api/binance/candles":"/api/twelvedata/candles",tf=customTf||$("tf").value,url=path+"?symbol="+encodeURIComponent(symbol)+"&interval="+intervalFromTf(src,tf)+"&outputsize=180",r=await fetch(url,{cache:"no-store",headers:{"Authorization":"Bearer "+token}}),d=await r.json();if(!r.ok||d.error)throw Error(d.error||"API error");let candles=d.values.map(v=>({time:v.datetime,open:+v.open,high:+v.high,low:+v.low,close:+v.close,volume:+(v.volume||1)})).filter(c=>Number.isFinite(c.close)).reverse();if(candles.length<40)throw Error("Candle < 40");return{candles,source:d.source||src}}
